...
with tab4:
    ...
    if time_option_tab4 == "Week":
        filtered_data['week_start_date'] = pd.to_datetime(filtered_data['started_at'], errors="coerce").dt.to_period('W').apply(lambda r: r.start_time)
        time_col_tab4 = "week_start_date"
    elif time_option_tab4 == "Month":
        filtered_data['month_name'] = pd.to_datetime(filtered_data['started_at'], errors="coerce").dt.strftime('%B')
        filtered_data['month_order'] = pd.to_datetime(filtered_data['started_at'], errors="coerce").dt.month
    ...
    if time_option_tab4 == "Month":
        grouped_data = filtered_data.groupby([time_col_tab4, 'month_order', 'Full_Name'])['Full_Name'].count().reset_index(name='Entry_Count')
        grouped_data.sort_values(by='month_order', inplace=True)
    else:
