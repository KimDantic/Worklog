...
with tab4:
    ...
    filtered_data_tab4 = filtered_data.copy()

    # Create a new column based on selected time option
    if time_option_tab4 == "Week":
        filtered_data_tab4['week_start_date'] = pd.to_datetime(filtered_data_tab4['started_at'], errors="coerce").dt.to_period('W').apply(lambda r: r.start_time)
        filtered_data_tab4['week_label'] = filtered_data_tab4['week_start_date'].dt.strftime('%Y-%m-%d')  # e.g., "2024-11-18"
        time_col_tab4 = "week_label"
        grouped_data = filtered_data_tab4.groupby([time_col_tab4, 'Full_Name'])['Full_Name'].count().reset_index(name='Entry_Count')

    elif time_option_tab4 == "Month":
        filtered_data_tab4['month_year'] = pd.to_datetime(filtered_data_tab4['started_at'], errors="coerce").dt.strftime('%B-%Y')  # e.g., "January-2025"
        filtered_data_tab4['month_order'] = pd.to_datetime(filtered_data_tab4['started_at'], errors="coerce").dt.to_period('M').apply(lambda r: r.start_time)
        time_col_tab4 = "month_year"

        grouped_data = filtered_data_tab4.groupby([time_col_tab4, 'Full_Name'])['Full_Name'].count().reset_index(name='Entry_Count')
    else:
        filtered_data_tab4['year_str'] = filtered_data_tab4['year'].astype(str)
        time_col_tab4 = "year_str"
        grouped_data = filtered_data_tab4.groupby([time_col_tab4, 'Full_Name'])['Full_Name'].count().reset_index(name='Entry_Count')

    fig = px.histogram(grouped_data, x=time_col_tab4, y="Entry_Count", title=f"Task Over time by {time_option_tab4}",
                       color="Full_Name", color_discrete_sequence=px.colors.qualitative.Plotly, barmode='group',
                       category_orders={time_col_tab4: sorted(grouped_data[time_col_tab4].unique())})

    fig.update_layout(
        plot_bgcolor='white',
        xaxis_title=time_option_tab4,
        yaxis_title='Task Entries',
        xaxis_tickangle=-45,
        xaxis_type='category'  # 👈 this line forces categorical x-axis (no 2024.5)
    )
